...
if "__main__":
    ...
    temperatures = range(76, 200, 2)
    Tcenter = []
    deltaT =  [100]
    Trange = [ [200,100] ]
    chem_pots = [ [mu/10000] for mu in range(3010, 3030, 1) ]
    ...
    cFile_ini.gc_period       = 1
    #cFile_ini.show_progress   = "true"
    
    lFile = logFile(cFile_ini.file_name_base)
	
    cFiles = []
    for i in range( len(chem_pots) ):

        cFiles.append( deepcopy(cFile_ini) )
        cFiles[-1].gc_chempots             = chem_pots[i]

        if len(Tcenter) > 0:
            temperatures = list(chain( range( Trange[i][1] + Tcenter[i], Tcenter[i], -deltaT[i] ),
                                 range( Tcenter[i] - deltaT[i], Tcenter[i] - Trange[i][0] - deltaT[i], -deltaT[i] ) ))
        cFiles[-1].temperature = temperatures[0]
