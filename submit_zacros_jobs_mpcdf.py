...
if "__main__":
    ...
    temperatures = np.arange(150, 501, 10)
    # temperature = [70, 200]

    coverages = np.arange(0.1, 0.3, 0.1)
    coverages = [0.11]

    list_of_clusters = [['0', '1nn', '2nn', '3nn', '4nn', '5nn', '6nn', '7nn', '8nn', '9nn',
                        '1-1-1', '1-1-2', '1-1-3', '2-2-5', '2-2-6', '3-3-3', '1-2-3']]
    list_of_energies = [ [ const.cl_data_fn_ce_3[s] for s in lst ] for lst in list_of_clusters ]
    ...
    infiles.run_type = ""
    infiles.header   = "O-Pt(111) 3-body"

    # simulation input parameters
    infiles.n_trajs        = 10
    infiles.first_traj     = 1
    infiles.snapshots      = 2000
    infiles.max_steps      = 100000
    infiles.wall_time      = 23*3600  # in seconds
      ...
      for T in temperatures:
        for cov in coverages:
          ...
          with open(infiles.wdir / "job.sh", "w") as f:
            ...
            f.write("\n")
            f.write(f"#SBATCH -D ./\n")
            f.write(f"#SBATCH -J {infiles.wdir.name}|zacros\n")
            f.write(f"#SBATCH --array=1-{infiles.n_trajs}\n")
            f.write(f"#SBATCH --time=23:30:00\n")
            f.write(f"#SBATCH --nodes=1\n") 
            f.write(f"#SBATCH --ntasks=1\n")
            f.write(f"#SBATCH --mem=1G\n")
            f.write(f"#SBATCH --mail-user=akandra@mpinat.mpg.de\n")
            f.write(f"#SBATCH --mail-type=none\n")
            f.write(f"# SBATCH -o output\n")
            f.write(f"\n")
            f.write(f"module purge\n")
            f.write(f"module load intel/2025.3 impi mkl\n")
            f.write(f"\n")   
            f.write(f"ulimit -s unlimited\n")
            f.write(f"export MKL_NUM_THREADS=1\n")
            f.write(f"export OMP_NUM_THREADS=1\n")
            f.write("# export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:${MKLROOT}/lib/intel64/\n")
            f.write("# export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:${INTEL_HOME}/lib/intel64/\n")
            ...
            f.write('srun /u/akandra/zacros/zacros_4.0/build/zacros.x > "zacros.out"' + '\n')
